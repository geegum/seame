...
def initialize_trackbars():
    ...
    initial_values = [50, 12, 255, 0, 255, 162]
...
while True:
    ...
    steering_d = np.clip(line_direction / 20, -1, 1)

    #Throttle = np.clip(0.21 - (abs(line_direction) / 500), 0.2, 0.21)
    Throttle = 0.2
    steering_publisher.publish(roslibpy.Message({'data': steering_d}))
    throttle_publisher.publish(roslibpy.Message({'data': Throttle}))

    print(f"lineDirection: {line_direction}, steering_d: {steering_d}, Throttle: {Throttle}")
    cv2.imshow('camera', frame_resized)
    cv2.imshow('mask', mask)

    if cv2.waitKey(1) != -1:
        steering_publisher.publish(roslibpy.Message({'data': 0}))
        throttle_publisher.publish(roslibpy.Message({'data': 0}))
        break
